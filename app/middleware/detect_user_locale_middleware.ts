...
export default class DetectUserLocaleMiddleware {
  ...
  protected getSupportedCookieLocale(ctx: HttpContext) {
    const cookieLocale = ctx.request.plainCookie('locale', { encoded: false })
    return i18nManager.getSupportedLocaleFor(cookieLocale)
  }
  ...
  async handle(ctx: HttpContext, next: NextFn) {
     ...
     */
    const language = this.getSupportedCookieLocale(ctx) ?? this.getRequestLocale(ctx)
    ...
    // Set cookie with a supported locale
    ctx.response.plainCookie('locale', ctx.i18n.locale, { encode: false, httpOnly: false })
